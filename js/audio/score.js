...
(function (C) {
  ...
  const THEME = {
    flora: [
      ...
      [['A4', 0, 3], ['C#5', 3, 3], ['G5', 6, 2], ['F#5', 8, 1], ['E5', 9, 3]],
  ...
  S.build = function (show) {
    ...
    for (const tr of show.turns) {
      ...
      if (tr.kind === 'cover') {
        ...
        for (const sx of tr.sheets) {
          const last = sx.j === sx.m - 1;
          at(sx.s0, (G, w) => I.swish(G, w, sx.s1 - sx.s0, last ? 0.7 : 0.42, 0.6, -0.6));
          if (!last) at(sx.s1 - 0.04, (G, w) => I.tick(G, w, 0.12, -0.5, false));
        }
